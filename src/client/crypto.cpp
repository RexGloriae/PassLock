...
std::vector<char> Crypto::aes_256_cbc_encr(const std::vector<char>& plain_text) {
    EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new();
    if (!ctx) throw Exception("Crypto err", 0x01);
    ...
    if (EVP_EncryptInit_ex(
            ...
            EVP_aes_256_cbc(),
            NULL,
            reinterpret_cast<const unsigned char*>(m_client_key.data()),
            reinterpret_cast<const unsigned char*>(m_client_iv.data())) != 1)
        throw Exception("Crypto err", 0x01);
...
std::vector<char> Crypto::aes_256_cbc_decr(const std::vector<char>& cipher_text) {
    EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new();
    if (!ctx) throw Exception("Crypto err", 0x01);
...
void Crypto::set_user_hash(const std::string& hash) {
    this->m_pass_hash = hash;
}

void Crypto::set_user_salt(const std::vector<char>& salt) {
    this->m_salt = salt;
    set_client_key();
    set_client_iv();
}

void Crypto::set_client_key() {
    std::vector<char> pbkdf;
    Crypto::get().gen_pbkdf2(m_pass_hash, m_salt, m_salt.size(), m_salt.size(), pbkdf);
    m_client_key = Crypto::get().gen_aes_key(pbkdf);
}
void Crypto::set_client_iv() {
    m_client_iv = Crypto::get().gen_aes_iv(m_client_key);
}
